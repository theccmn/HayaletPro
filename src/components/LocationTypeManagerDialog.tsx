...
import { createLocationType, updateLocationType, deleteLocationType, updateLocationTypeOrder, getLocationTypes } from '../services/apiLocationTypes';
import { Plus, Trash2, GripVertical, Check, MapPin } from 'lucide-react';
...
interface LocationTypeManagerDialogProps {
    isOpen: boolean;
    onClose: () => void;
}

export function LocationTypeManagerDialog({ isOpen, onClose }: LocationTypeManagerDialogProps) {
    const queryClient = useQueryClient();
    const [newTypeLabel, setNewTypeLabel] = useState('');
    const [newTypeColor, setNewTypeColor] = useState('bg-purple-100 text-purple-700');
    ...
    const [localTypes, setLocalTypes] = useState<LocationType[]>([]);
    ...
    const createMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['location_types'] });
            toast.success('Mekan türü eklendi');
        },
        onError: (error) => {
            toast.error('Mekan türü eklenirken hata: ' + error.message);
        }
    });

    const updateMutation = useMutation({
        mutationFn: ({ id, updates }: { id: string; updates: Partial<LocationType> }) =>
            updateLocationType(id, updates),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['location_types'] });
            toast.success('Mekan türü güncellendi');
        },
        onError: (error) => {
    ...
    const sortMutation = useMutation({
        mutationFn: updateLocationTypeOrder,
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['location_types'] })
    });
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['location_types'] });
            toast.success('Mekan türü silindi');
        },
        onError: (error) => {
    ...
    const handleCreate = () => {
        ...
        createMutation.mutate({
            label: newTypeLabel,
            order_index: (types?.length || 0),
            color: newTypeColor
        });
        setNewTypeLabel('');
        setNewTypeColor('bg-purple-100 text-purple-700');
    };
    ...
    return (
        <>
            <Dialog isOpen={isOpen} onClose={onClose} title="Çekim Mekanı Türü Yönetimi" description="Mekan türlerini ekleyin, düzenleyin veya sıralayın.">
                <div className="mt-4 space-y-4">
                    <div className="flex gap-2">
                        <Input
                            placeholder="Yeni mekan türü adı..."
                            value={newTypeLabel}
                    ...
                    {localTypes.length === 0 && (
                        <div className="text-center py-8 text-muted-foreground">
                            ...
                            <p className="text-sm">Henüz mekan türü eklenmedi</p>
                        </div>
                    )}
                </div>
            </Dialog>
            ...
            <AlertDialog open={!!typeToDelete} onOpenChange={(open) => !open && setTypeToDelete(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Mekan Türünü Sil</AlertDialogTitle>
                        <AlertDialogDescription>
                            Bu mekan türünü silmek istediğinize emin misiniz? Bu işlem geri alınamaz.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
